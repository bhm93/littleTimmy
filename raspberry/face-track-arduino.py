#!/usr/bin/env python
progname = "face-track-arduino.py"
ver = "ver 0.93"
...
if debug:
    print("Angular vo Settings for gpiozero")
    print("-----------------------------------")
    ...
    while still_scanning:
        ...
        if check_timer(motion_start, timer_motion): 
            ...
            if face_data != ():
                ...
                strx = '%0*d' % (3, int(Nav_LR))
                stry = '%0*d' % (3, int(Nav_UD))
                if debug:            
                    print("face-track - Found Face at pan_cx=%3i pan_cy=%3i Nav_LR=%3i Nav_UD=%3i "
                                                   % (pan_cx, pan_cy, Nav_LR, Nav_UD))
                    print("face-track - Found Face at cx=%s cy=%s "
                                                   % (strx, stry))
                    
                pan_cx, pan_cy = pan_goto(pan_cx, pan_cy)
                
                if (Nav_LR > 3 or Nav_LR < -3) or (Nav_UD > 3 or Nav_UD < -3): 
                    if ser.in_waiting:
                        puedoLeer = ser.readline()
                        print(puedoLeer)
                        if puedoLeer.strip() == 'Y':
                            print("ENVIO")
                            ser.write('S' + strx + stry)
                            print(ser.readline())
                        else:
                            print("NO ENVIO")
                #print(ser.readline())
